...
function compile(grammar, source) {
  ...
  const vars = localVars(matchResult);
  const mainFn = func(
    [locals(vars.length, valtype.i32)],
    toWasm(matchResult, vars)
  );
  const mod = module([
...
async function compileAndEval(input) {
  const { instance } = await WebAssembly.instantiate(compile(wafer, input));
  return instance.exports.main();
}

test('module with locals & assignment', async () => {
  assert.equal(await compileAndEval('let x = 42; x'), 42);
  assert.equal(await compileAndEval('let a = 13; let b = 15; a + b'), 28);
});
